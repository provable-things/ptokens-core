...
use crate::{
    chains::eos::{
        ...
        eos_state::EosState,
        eos_types::{Checksum256s, EosBlockHeaderJson, EosSignedTransaction, EosSignedTransactions},
        eos_utils::{convert_bytes_to_checksum256, convert_hex_to_checksum256},
        parse_eos_schedule::{
...
use secp256k1::{key::SecretKey, Message as Secp256k1Message};
use std::{fs::read_to_string, path::Path, str::FromStr};

pub const SAMPLE_EOS_BLOCK_AND_ACTION_JSON_PATH_1: &str = "src/btc_on_eos/eos/eos_test_utils/eos-block-81784220.json";

pub const SAMPLE_EOS_BLOCK_AND_ACTION_JSON_PATH_2: &str = "src/btc_on_eos/eos/eos_test_utils/eos-block-80440580.json";

pub const SAMPLE_EOS_BLOCK_AND_ACTION_JSON_PATH_3: &str = "src/btc_on_eos/eos/eos_test_utils/eos-block-84187467.json";

pub const SAMPLE_EOS_BLOCK_AND_ACTION_JSON_PATH_4: &str = "src/btc_on_eos/eos/eos_test_utils/eos-block-81772484.json";

pub const SAMPLE_EOS_BLOCK_AND_ACTION_JSON_PATH_5: &str = "src/btc_on_eos/eos/eos_test_utils/eos-block-10800.json";

pub const SAMPLE_EOS_BLOCK_AND_ACTION_JSON_PATH_6: &str =
    "src/btc_on_eos/eos/eos_test_utils/jungle-3-block-8242000.json";

pub const SAMPLE_EOS_BLOCK_AND_ACTION_JSON_PATH_7: &str = "src/btc_on_eos/eos/eos_test_utils/eos-block-10700626.json";
...
pub fn get_sample_v1_schedule_json_string() -> Result<String> {
    Ok(read_to_string(
        "src/btc_on_eos/eos/eos_test_utils/sample-schedule-389-v1.json",
    )?)
}

pub fn get_sample_v2_schedule_json_string() -> Result<String> {
    Ok(read_to_string(
        "src/btc_on_eos/eos/eos_test_utils/sample-schedule-28-v2.json",
    )?)
}
...
pub fn get_sample_mainnet_schedule_1713() -> Result<EosProducerScheduleV2> {
    parse_v1_schedule_string_to_v1_schedule_json(&read_to_string(
        "src/btc_on_eos/eos/eos_test_utils/sample-schedule-1713-v1.json",
    )?)
...
pub fn get_sample_j3_schedule_37() -> Result<EosProducerScheduleV2> {
    parse_v1_schedule_string_to_v1_schedule_json(&read_to_string(
        "src/btc_on_eos/eos/eos_test_utils/sample-j3-schedule-37.json",
    )?)
    .and_then(|v1_json| convert_v1_schedule_json_to_v1_schedule(&v1_json))
    .map(|v1_schedule| convert_v1_schedule_to_v2(&v1_schedule))
}
